...
def get_projects():
    ...
    # --- START: The Optimized Query ---
    # Subquery to calculate total paid amount per project
    paid_subquery = db.session.query(
        Invoice.project_id,
        func.sum(Payment.amount).label('total_paid')
    ).join(Payment).group_by(Invoice.project_id).subquery()

    # Main query to fetch projects and calculate all costs in one go
    query = db.session.query(
        Project,
        func.coalesce(func.sum(Item.contract_quantity * Item.contract_unit_cost), 0).label('total_contract_cost'),
        func.coalesce(func.sum(
            case(
                (Item.actual_quantity * Item.actual_unit_cost > 0, Item.actual_quantity * Item.actual_unit_cost),
                else_=db.session.query(func.sum(Payment.amount)).join(InvoiceItem, Payment.invoice_item_id == InvoiceItem.id).filter(InvoiceItem.item_id == Item.id).scalar_subquery()
            )
        ), 0).label('total_actual_cost'),
    ...
    # Process results to attach calculated values to project objects
    projects_with_costs = []
